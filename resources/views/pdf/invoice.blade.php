...
<head>
    ...
    <style>
        ...
        .header .company-name { font-size: 18pt; font-weight: bold; color: #2563eb; }
        .header .invoice-title { font-size: 16pt; font-weight: bold; text-align: right; color: #1e293b; }
        ...
        table.items th { background: #2563eb; color: #fff; padding: 8px 10px; text-align: left; font-size: 9pt; text-transform: uppercase; letter-spacing: 0.5px; }
        ...
        .totals .grand-total td { font-size: 12pt; font-weight: bold; border-top: 2px solid #2563eb; padding-top: 8px; }
...
<body>
        ...
        <table style="width:100%">
            <tr>
                ...
                <td style="width:50%;text-align:right">
                    ...
                    <div style="font-size:10pt;color:#64748b;margin-top:4px">N° {{ $invoice->number }}</div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="totals">
        <tr>
            ...
            <td>{{ $money::format($invoice->subtotal_xof) }}</td>
        </tr>
        @if($invoice->tax_xof > 0)
            <tr>
                <td style="text-align:right">Total TVA</td>
                <td>{{ $money::format($invoice->tax_xof) }}</td>
            </tr>
        @endif
        @if($invoice->discount_xof > 0)
            <tr>
                <td style="text-align:right">Remise{{ $invoice->discount_type ? ' (' . $invoice->discount_type . ')' : '' }}</td>
                <td>-{{ $money::format($invoice->discount_xof) }}</td>
            </tr>
        @endif
        <tr class="grand-total">
            <td style="text-align:right">Total</td>
            <td>{{ $money::format($invoice->total_xof) }}</td>
        </tr>
        @if($invoice->paid_xof > 0)
            <tr>
                <td style="text-align:right;color:#166534">Déjà payé</td>
                <td style="color:#166534">{{ $money::format($invoice->paid_xof) }}</td>
            </tr>
            <tr class="grand-total">
                <td style="text-align:right">Solde dû</td>
                <td>{{ $money::format($invoice->balance_due_xof) }}</td>
            </tr>
        @endif
    </table>
