  ...
  <AdminLayout>
    <div class="space-y-4">
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        <BaseInput v-model="search" :placeholder="$t('page.warehouses.search_placeholder')" clearable size="sm" class="flex-1 max-w-xs" />
      ...
      <template v-else-if="warehouses.length === 0">
        <BaseEmptyState :title="$t('page.warehouses.title')" :description="$t('page.warehouses.empty')">
          <BaseButton variant="primary" size="sm" :to="{ name: 'WarehouseCreate' }">
            <span class="text-white"><PlusIcon class="w-4 h-4" /></span>{{ $t('page.warehouses.create') }}
          </BaseButton>
        </BaseEmptyState>
      ...
      <template v-else>
        <BaseTable
          :columns="[
            { key: 'name', label: $t('page.warehouses.name'), sortable: true },
            { key: 'code', label: $t('page.warehouses.code'), sortable: true, class: 'hidden md:table-cell' },
            { key: 'address', label: $t('form.address'), class: 'hidden md:table-cell' },
            { key: 'city', label: $t('page.warehouses.city'), sortable: true, class: 'hidden lg:table-cell' },
        ...
        >
          ...
          <template #cell-address="{ row }">
            <span class="text-sm text-text-secondary">{{ row.address || '—' }}</span>
          </template>
          <template #cell-city="{ row }">
            <span class="text-sm text-text-secondary">{{ row.city || '—' }}</span>
          </template>
          <template #cell-is_active="{ row }">
            <span :class="['inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full border', row.is_active ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-500 border-gray-200']">
              <span class="w-1.5 h-1.5 rounded-full" :class="row.is_active ? 'bg-green-500' : 'bg-gray-400'"></span>
          ...
          <template #actions="{ row }">
            <BaseDropdown align="right">
              ...
              <template #menu>
                <router-link :to="{ name: 'WarehouseEdit', params: { id: row.id } }" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-text-primary hover:bg-surface-secondary transition-colors">
    ...
    <BaseModal v-model="deleteTarget" :title="$t('common.confirm_delete')" size="sm">
      <p class="text-sm text-text-secondary">{{ $t('page.warehouses.delete_confirm', { name: deleteTarget?.name }) }}</p>
      <template #footer>
...
const sortedWarehouses = computed(() => {
  if (!sortBy.value) return warehouses.value;
...
async function fetchWarehouses(page) {
  loading.value = true;
  try {
    const params = { page: page || 1, per_page: perPage.value };
    if (search.value) params.search = search.value;
    const { data } = await axios.get('/warehouses', { params });
    warehouses.value = data.data ?? data;
    meta.value = data.meta ?? null;
  } catch { showToast($t('page.warehouses.load_error'), 'error'); } finally { loading.value = false; }
}
...
watch([search, perPage], () => {
  ...
  debounceTimer = setTimeout(fetchWarehouses, 300);
});
...
function confirmDelete(w) { deleteTarget.value = w; }
...
async function executeDelete() {
  ...
  try {
    ...
    showToast($t('page.warehouses.deleted', { name: w.name }), 'success');
    const isLastItemOfLastPage = warehouses.value.length === 1 && meta.value?.last_page > 1 && currentPage === meta.value.last_page;
    fetchWarehouses(isLastItemOfLastPage ? currentPage - 1 : currentPage);
  } catch (err) {
    if (err.response?.status === 409 || err.response?.data?.message) {
      showToast(err.response.data.message || $t('page.warehouses.delete_blocked'), 'error');
    } else { showToast($t('common.delete_error'), 'error'); }
  }
}
</script>
