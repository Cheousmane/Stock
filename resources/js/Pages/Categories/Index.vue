<template>
  <AdminLayout>
    <div class="space-y-4">
      <BasePageHeader eyebrow="Catalogue" :title="$t('page.categories.title')" :subtitle="meta ? $t('page.categories.total_count', { count: meta.total }) : undefined">
        <template #actions>
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        <BaseInput v-model="search" :placeholder="$t('page.categories.search_placeholder')" clearable size="sm" class="flex-1 max-w-xs" />
...
async function fetchCategories(page) {
  ...
  } catch { showToast($t('page.categories.load_error'), 'error'); } finally { loading.value = false; }
}
...
async function executeDelete() {
  ...
  try {
    ...
    const isLastItemOfLastPage = categories.value.length === 1 && meta.value?.last_page > 1 && currentPage === meta.value.last_page;
    fetchCategories(isLastItemOfLastPage ? currentPage - 1 : currentPage);
