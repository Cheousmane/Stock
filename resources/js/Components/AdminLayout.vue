    ...
    >
      ...
      <div class="flex items-center h-14 px-3 border-b border-sidebar-border shrink-0 sidebar-transition">
        ...
        <router-link to="/dashboard" class="flex items-center gap-2.5 min-w-0">
          <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-400 to-emerald-600 shadow-lg shadow-emerald-500/20 shrink-0 transition-transform duration-300 hover:scale-125">
            <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
            </svg>
          </span>
          <div class="sidebar-label overflow-hidden transition-all duration-200 ease-[cubic-bezier(0.25,0.1,0.25,1)]" :class="sidebarCollapsed ? 'w-0 opacity-0 -translate-x-2' : 'w-auto opacity-100 translate-x-0'">
            <div class="min-w-0 whitespace-nowrap">
              ...
              <p class="text-[10px] font-medium text-sidebar-text-secondary truncate tracking-wider sidebar-transition">SIDIBE CORPORATE</p>
            </div>
          </div>
        </router-link>
        <button
          @click="sidebarCollapsed = !sidebarCollapsed"
          class="hidden lg:flex items-center justify-center ml-auto w-6 h-6 rounded-md text-sidebar-text-secondary hover:text-sidebar-text hover:bg-sidebar-surface-hover sidebar-transition transition-transform duration-300"
      ...
      <nav class="flex-1 px-2 py-3 overflow-y-auto overflow-x-hidden">
        <div class="space-y-0.5">
          <template v-for="item in navItems" :key="item.name || item.key">
            <div v-if="item.divider" class="sidebar-label overflow-hidden transition-all duration-200 ease-[cubic-bezier(0.25,0.1,0.25,1)]" :class="sidebarCollapsed ? 'w-0 opacity-0 h-0 pt-0 pb-0' : 'w-auto opacity-100 px-3 pt-4 pb-1.5'">
            ...
            <router-link
              v-if="!item.divider"
              :to="item.to"
              class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 group sidebar-transition"
            ...
            >
              ...
              <span class="sidebar-label truncate overflow-hidden transition-all duration-200 ease-[cubic-bezier(0.25,0.1,0.25,1)]" :class="sidebarCollapsed ? 'w-0 opacity-0 -translate-x-2' : 'w-auto opacity-100 translate-x-0'">{{ item.label || $t(item.key) }}</span>
              <span v-if="item.badge" class="sidebar-label flex-shrink-0 transition-all duration-200 ease-[cubic-bezier(0.25,0.1,0.25,1)]" :class="sidebarCollapsed ? 'w-0 opacity-0 scale-0' : 'w-auto opacity-100 scale-100'">
      ...
      <div class="p-2 border-t border-sidebar-border sidebar-transition">
        <button
          @click="logout"
          class="flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm font-medium text-sidebar-text-secondary hover:text-red-400 hover:bg-red-500/10 transition-colors group sidebar-transition"
        ...
        >
          ...
          <span class="sidebar-label truncate overflow-hidden transition-all duration-200 ease-[cubic-bezier(0.25,0.1,0.25,1)]" :class="sidebarCollapsed ? 'w-0 opacity-0 -translate-x-2' : 'w-auto opacity-100 translate-x-0'">{{ $t('common.logout') }}</span>
        </button>
      </div>
    </aside>
    ...
    <Teleport to="body">
      <Transition name="tooltip">
        <div
          ...
          class="fixed z-[99999] px-2.5 py-1.5 text-xs font-medium bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900 rounded-lg shadow-lg whitespace-nowrap pointer-events-none border border-white/10 dark:border-gray-300 -translate-y-1/2"
        >
          {{ tooltip.text }}
        </div>
      </Transition>
      ...
      <header class="sticky top-0 z-30 flex items-center h-14 px-4 lg:px-6 bg-surface/70 backdrop-blur-xl border-b border-border">
        ...
        <div class="flex items-center gap-2 ml-auto">
          ...
          <button
            ...
            class="flex items-center gap-2 px-3 py-1.5 text-xs text-text-tertiary bg-surface-tertiary rounded-lg hover:bg-surface-tertiary/80 transition-colors hidden sm:flex"
          >
          ...
          <div class="relative" v-click-outside="() => dropdownOpen = false">
            <button
              ...
              class="flex items-center gap-2 px-2 py-1.5 text-sm font-medium text-text-secondary rounded-lg transition-colors hover:bg-surface-tertiary"
            >
              ...
              <svg class="w-3.5 h-3.5 text-text-tertiary transition-transform duration-200" :class="{ 'rotate-180': dropdownOpen }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <Transition name="dropdown">
              <div v-if="dropdownOpen" class="absolute right-0 z-50 mt-2 w-56 bg-surface border border-border rounded-xl shadow-dropdown py-1 overflow-hidden">
                ...
                <router-link to="/settings" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-text-secondary hover:bg-surface-tertiary transition-colors">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <button @click="logout" class="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm text-left text-red-600 hover:bg-red-50 transition-colors dark:text-red-400 dark:hover:bg-red-500/10">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
                  </svg>
                  {{ $t('common.logout') }}
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="searchOpen" class="fixed inset-0 z-[200] flex items-start justify-center pt-[15vh]" @click.self="searchOpen = false">
          ...
          <div class="relative w-full max-w-xl bg-surface border border-border rounded-2xl shadow-modal overflow-hidden animate-scale-in">
            ...
            <div class="p-2 max-h-80 overflow-y-auto">
              ...
              <div v-else class="space-y-0.5">
                <div v-for="r in searchResults" :key="r.url" @click="navigateToSearch(r)" class="flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer hover:bg-surface-tertiary transition-colors">
                  <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-surface-tertiary shrink-0">
...
onMounted(async () => {
  ...
  try {
    ...
    companyName.value = c.company_name || c.name || ''
  } catch {}
})
</script>
...
.tooltip-enter-active, .tooltip-leave-active { transition: opacity 0.1s ease; }
